...
void Camera::init()
{
	...
	rotate(this->yawPitchRoll.x, this->yawPitchRoll.y, this->yawPitchRoll.z);	//Makes sure the rotation is right in the beginning
...
void Camera::input(Display * display)
{
	if (this->active) //If current camera is the active one
	{
		...
		if (isCClicked) //If C is clicked, activate camera mouse movement
		{
			...
			{
				this->yawPitchRoll.x += (float)(xPos*dt*sensitivity);	//Calculate yaw with mouse x movement
				this->yawPitchRoll.y -= (float)(yPos*dt*sensitivity);	//Calculate pitch with mouse y movemnt

				if (yawPitchRoll.y > glm::radians(89.0f))	//Lock the mouse for inverting on itself
					yawPitchRoll.y = glm::radians(89.0f);
				if (yawPitchRoll.y < -glm::radians(89.0f))
					yawPitchRoll.y = -glm::radians(89.0f);
				
				rotate(this->yawPitchRoll.x, this->yawPitchRoll.y, this->yawPitchRoll.z);	//Update the rotation with the new yaw and pitch (roll isn't being used)
			}
...
void Camera::lookAt(const glm::vec3 & target)
{
	...
	this->yawPitchRoll.x = atan2(this->f.z, this->f.x);			//Calculate the yaw pitch and roll with the new vectors
...
{
	this->f.x = cos(yaw)*cos(pitch);								//calculate new f with the rotations in mind
	this->f.y = sin(pitch);
	this->f.z = sin(yaw)*cos(pitch);
	this->r = glm::normalize(glm::cross(f, GLOBAL_UP_VECTOR));
	this->u = glm::cross(r, f);

	this->yawPitchRoll.x = yaw;										//If the function takes in yaw pitch and roll which is != yawPitchRoll.xyz, the update
